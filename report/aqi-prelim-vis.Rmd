...
gg_seasquantile(aqi_data, q = seq(.025, .975, .005)) +
  labs(
    x = "Month", y = "AQI",
    title = "2.5~97.5% Quantile"
  )
```
...
gg_seasquantile(aqi_data, period = "day", q = seq(.025, .975, .005)) +
  theme(axis.text.x = element_text(angle = 90, vjust = .5)) +
  labs(
    x = "Time of Day", y = "AQI",
    title = "2.5~97.5% Quantile"
...
filter(aqi_data, location == "queen_street") %>%
  ...
  gg_seasquantile(q = seq(.025, .975, .005), period = "day") +
  theme(axis.text.x = element_text(angle = 90, vjust = .5)) +
  labs(
    x = "Time of Day", y = "AQI",
    title = "2.5~97.5% Quantile of AQI on Queen Street"
  )
```
...
```{r location}
aqi_data %>%
  gg_seasquantile(q = seq(0, .95, .005), polar = TRUE) +
  facet_geo(~location, grid = akl_city_grid) +
  labs(
    title = "0~95% Quantile of AQI by Relative Geolocation",
    y = "AQI"
  )
```
