...
gg_seasquantile(aqi_data, q = seq(.1, .9, .005)) +
  labs(
    x = "Month", y = "AQI",
...
gg_seasquantile(aqi_data, period = "day", q = seq(.1, .9, .005)) +
  theme(axis.text.x = element_text(angle = 90, vjust = .5)) +
  labs(
    x = "Time of Day", y = "AQI",
    title = "10~90% Quantile"
...
filter(aqi_data, location == "queen_street") %>%
  ...
  gg_seasquantile(q = seq(.1, .9, .005), period = "day") +
  theme(axis.text.x = element_text(angle = 90, vjust = .5)) +
  labs(
    x = "Time of Day", y = "AQI",
...
aqi_data %>%
  gg_seasquantile(q = seq(.1, .9, .005), polar = TRUE) +
